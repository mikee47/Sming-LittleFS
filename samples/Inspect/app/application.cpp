...
bool copyFileSystem(IFS::IFileSystem* srcfs, IFS::IFileSystem* dstfs)
{
...
void fstest()
{
	...
	auto fs = reinterpret_cast<IFS::LittleFS::FileSystem*>(getFileSystem());
	Serial.print(F("Perf stats: "));
	Serial.println(fs->perfStat.toString());

	IFS::FileSystem::Info info;
	fs->getinfo(info);
	Serial.print(F("Volume Size: "));
	Serial.print(info.volumeSize);
	Serial.print(F(", Used: "));
	Serial.print(info.volumeSize - info.freeSpace);
	Serial.print(F(", Free space: "));
	Serial.print(info.freeSpace);
	Serial.println();
}
